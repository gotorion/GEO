...
#### 获取数据集 ####
eset1 <- getGEO('GSE15471', destdir = ".", getGPL = F)
eset2 <- getGEO('GSE16515', destdir = ".", getGPL = F)
#### 提取表达矩阵 ####
exp1 <- exprs(eset1[[1]])
exp2 <- exprs(eset2[[1]])
range(exp1)
...
col <- c(rep('red', 78), rep('blue', 52))
par(mar = c(8, 3, 1, 2) + 0.1)
boxplot(eset_merge, col=col, las=2)
dev.off()
#### 分组 ####
group1 = ifelse(str_detect(pd1$characteristics_ch1.1, "normal"), "Normal", "Tumor")
group2 = ifelse(str_detect(pd2$title, "Normal"), "Normal", "Tumor")
table(group1)
table(group2)
...
#### 去除批次效应 ####
GSE <- c(rep('GSE15471', 78), rep('GSE16515', 52))
GSE
table(group_list, GSE)
library(limma)
data <- eset_merge
batch <- c(rep('GSE15471', 78), rep('GSE16515', 52))
